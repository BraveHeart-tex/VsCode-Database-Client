  ...
  bus
    .on('CONNECT', async (config) => {
      try {
        ...
        bus.send({ type: 'CONNECTION_SUCCESS', payload: { connectionId: config.id } })
      } catch (err: any) {
        bus.send({ type: 'CONNECTION_ERROR', payload: { message: err.message } })
      }
    })
    .on('EXECUTE_QUERY', async ({ sql, connectionId }) => {
      try {
        ...
        bus.send({ type: 'QUERY_ERROR', payload: { message: err.message, queryId: '' } })
      }
    })
